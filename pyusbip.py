...
class USBIPConnection:
    ...
    def pack_device_desc(self, dev, interfaces=True):
        ...
        path = "pyusbip/{}/{}".format(dev.getBusNumber(),
        ...
        if interfaces:
            for ifc in config.iterInterfaces():
                set = list(ifc)[0]
                data += struct.pack(">BBBB", set.getClass(),
                                    set.getSubClass(), set.getProtocol(), 0)
    ...
    def handle_op_devlist(self):
        ...
        resp = struct.pack(">HHII", USBIP_VERSION, USBIP_OP_DEVLIST |
                           USBIP_REPLY, USBIP_ST_OK, len(devlist))
        for dev in devlist:
    ...
    def handle_op_import(self, busid):
        ...
        devlist = usbctx.getDeviceList()
        for dev in devlist:
            dev_busid = "{}-{}".format(dev.getBusNumber(),
                                       dev.getDeviceAddress())
            if busid == dev_busid:
                hnd = dev.open()
        ...
        resp = struct.pack(">HHI", USBIP_VERSION,
                           USBIP_OP_IMPORT | USBIP_REPLY, USBIP_ST_NA)
        self.writer.write(resp)

    async def handle_urb_submit(self, seqnum, dev, direction, ep):
        op_submit = ">Iiiii8s"
        data = await self.reader.readexactly(struct.calcsize(op_submit))
        (transfer_flags, buflen, start_frame, number_of_packets,
         interval, setup) = struct.unpack(op_submit, data)
        ...
        (bRequestType, bRequest, wValue, wIndex,
         wLength) = struct.unpack("<BBHHH", setup)

        self.say("seq {:x}: ep {}, direction {}, {} bytes".format(
            seqnum, ep, direction, buflen))
        ...
        if ep == 0:
            ...
            self.say("EP0 requesttype {}, request {}".format(
            ...
            if bRequestType == USB_RECIP_DEVICE and bRequest == USB_REQ_SET_ADDRESS:
                ...
                for i in range(config.getNumInterfaces()):
                    ...
                    try:
                        ...
                        print('    kernel driver check failed')
            ...
            if direction == USBIP_DIR_IN:
                def callback(xfer_):
                    self.say('callback IN seqnum {:x} status {} len {} buflen {}'.format(
                        seqnum, xfer.getStatus(), xfer.getActualLength(), len(xfer.getBuffer())))
                    resp = struct.pack(">IIIIIiiiii8s",
                                       USBIP_RET_SUBMIT, seqnum,
                                       0, 0, 0,
                ...
                def callback(xfer_):
                    self.say('callback OUT seqnum {:x} status {} '.format(
    ...
    async def handle_urb_unlink(self, seqnum, dev, direction, ep):
        op_submit = ">Iiiii8s"
        data = await self.reader.readexactly(struct.calcsize(op_submit))
        (sseqnum, buflen, start_frame, number_of_packets,
         interval, setup) = struct.unpack(op_submit, data)
    ...
    async def handle_packet(self):
        ...
        if version == 0x0000:
            ...
            if devid not in self.devices:
                raise USBIPProtocolErrorException(
                    'devid unattached {:x}'.format(devid))
...
def main():
    """Main entry point for the pyusbip server."""
